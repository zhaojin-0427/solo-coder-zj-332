...
import useStore, { Exhibition } from '@/store/useStore'
...
export default function ExhibitionForm({ open, onClose, exhibition }: ExhibitionFormProps) {
  const { createExhibition, updateExhibition } = useStore()
  ...
  useEffect(() => {
    if (exhibition) {
      setFormData({
        ...
        themeType: exhibition.themeType || '社区展览',
        description: exhibition.description || '',
        startDate: exhibition.startDate || '',
        endDate: exhibition.endDate || '',
        location: exhibition.location || '',
        status: exhibition.status || '草稿',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.name || !formData.createdBy) return
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(92,58,30,0.4)' }}>
      <div className="rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" style={{ background: '#FFFBF0' }}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-brown)' }}>展陈名称 *</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-brown)' }}>展陈描述</label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-brown)' }}>开始日期</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-brown)' }}>结束日期</label>
              <input
                type="date"
          ...
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-brown)' }}>展陈地点</label>
            <input
              type="text"
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={!formData.name || !formData.createdBy}

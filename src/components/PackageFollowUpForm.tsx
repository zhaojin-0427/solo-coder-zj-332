...
import useStore, { PackageFollowUp } from '@/store/useStore'

interface PackageFollowUpFormProps {
  open: boolean
  onClose: () => void
  packageId: string
  item?: PackageFollowUp | null
  mode?: 'edit' | 'add'
  packageItems?: { id: string; title: string }[]
}

export default function PackageFollowUpForm({ open, onClose, packageId, item, mode = 'add', packageItems }: PackageFollowUpFormProps) {
  const { addPackageFollowUp, updatePackageFollowUp } = useStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'edit' && item) {
      await updatePackageFollowUp(packageId, item.id, formData)
    } else {
      await addPackageFollowUp(packageId, formData)
    }
    onClose()
  }
